...
/*   Updated: 2021/01/19 17:49:48 by hyunjuyo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

void	do_printf_x_basic(char *ptr_n, char *ptr_t, t_opts *opt)
{
	int	i;
...
void	do_printf_x_py(char *ptr_n, char *ptr_t, t_opts *opt, t_flags *flags)
{
	int	i;
...
void	do_printf_x_pn(char *ptr_n, char *ptr_t, t_opts *opt, t_flags *flags)
{
	int	i;
	...
	else if (flags->iszero == 1)
	{
		...
		do_printf_x_basic(ptr_n, ptr_t, opt);
}

void	do_printf_x(char *ptr_n, t_opts *opt, t_flags *flags)
{
	char	*ptr_t;
	char	*temp;
	int		i;
	...
	ptr_n = temp;
	if (flags->prcs >= 0)
		do_printf_x_py(ptr_n, ptr_t, opt, flags);
	else
		do_printf_x_pn(ptr_n, ptr_t, opt, flags);
	free(ptr_n);
